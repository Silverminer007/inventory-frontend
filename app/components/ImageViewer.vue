...
function onKeydown(e: KeyboardEvent) {
  switch (e.key) {
    ...
    case '+':          scale.value = Math.min(scale.value * 1.2, 6); break
    case '-':          scale.value = Math.max(scale.value * 0.8, 1); if (scale.value <= 1) resetTransform(); break
  }
}
  ...
  >
    ...
    <div v-if="images.length > 1" class="flex justify-center items-center gap-1.5 py-4 shrink-0">
      <button
        ...
        :aria-label="`Bild ${i + 1}`"
        @click="currentIndex = i; resetTransform()"
      />
    </div>
  </div>
</template>

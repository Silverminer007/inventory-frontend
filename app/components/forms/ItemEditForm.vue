...
async function submit() {
  ...
  try {
    ...
    const updated = await commands.executeCommand<LocalItem>('ITEM_UPDATE', payload, props.item.id)
...
async function queueImages(itemId: number) {
  const isOnline = typeof navigator !== 'undefined' && navigator.onLine
  const hasRealId = itemId > 0
  for (const file of selectedFiles.value) {
    if (isOnline && hasRealId) {
      try {
...
async function onMove(newContainerId: number) {

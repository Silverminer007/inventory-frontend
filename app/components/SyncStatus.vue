...
const dotStyle = computed(() => {
  if (sync.isSyncing.value) return 'background: var(--color-warning); animation: pulse-dot 1.2s infinite'
  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="recentCommands.length > 0">
        <h4 class="text-xs font-semibold uppercase tracking-wide mb-2" style="color: var(--color-text-muted)">
          Letzte Aktionen
        </h4>
        <div class="space-y-2">
          <div
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs" style="color: var(--color-text-muted)">
                {{ new Date(cmd.createdAt).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }) }}
              </p>
            </div>
            <Badge :variant="(statusColorMap[cmd.status] ?? 'default') as 'amber' | 'blue' | 'green' | 'red' | 'default'">

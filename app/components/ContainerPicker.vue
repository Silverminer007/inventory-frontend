...
function shelvesOf(roomId: UUID): Container[] {
  return filtered.value.filter(c => c.containerType === 'SHELF' && c.parentContainerId === roomId)
}

function boxesOf(shelfId: UUID): Container[] {
  return filtered.value.filter(c => c.containerType === 'BOX' && c.parentContainerId === shelfId)
}
...
<template>
  <BottomSheet :title="title ?? 'Lagerort wählen'" @close="emit('close')">
    ...
    <div class="relative mb-3">
      <Icon icon="mdi:magnify" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none"
    ...
    <div class="space-y-1 max-h-[50vh] overflow-y-auto">
        ...
        <div>
          ...
          >
            ...
            >
              ...
              <span class="flex-1 font-medium text-sm truncate" style="color: var(--color-text-primary)">{{ room.name }}</span>
            </button>
            <button
              class="p-1 shrink-0"
              @click="expandedRooms.has(room.id) ? expandedRooms.delete(room.id) : expandedRooms.add(room.id)"
            >
              <Icon
          ...
          <template v-if="expandedRooms.has(room.id)">
            <template v-for="shelf in shelvesOf(room.id)" :key="shelf.id">
              ...
              >
                ...
                >
                  <Icon :icon="containerConfig.SHELF.icon" class="w-4 h-4 shrink-0" style="color: var(--color-accent)" />
                  <span class="flex-1 text-sm truncate" style="color: var(--color-text-primary)">{{ shelf.name }}</span>
                </button>
                <button
                  class="p-1 shrink-0"
                  @click="expandedShelves.has(shelf.id) ? expandedShelves.delete(shelf.id) : expandedShelves.add(shelf.id)"
                >
                  <Icon
              ...
              <template v-if="expandedShelves.has(shelf.id)">
                ...
                >
                  <Icon :icon="containerConfig.BOX.icon" class="w-4 h-4 shrink-0" style="color: var(--color-accent)" />
                  <span class="text-sm" style="color: var(--color-text-primary)">{{ box.name }}</span>
                </button>
              </template>
            </template>
          </template>
      ...
      <div v-if="rooms.length === 0" class="py-8 text-center text-sm" style="color: var(--color-text-muted)">

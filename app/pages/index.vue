<script setup lang="ts">
  ...
  function childCount(container: Container): { containers: number; items: number } {
    const containers = allContainers.value.filter(
  ...
  function childLabel(container: Container): string {
    ...
    if (containers > 0) {
      ...
      parts.push(
        `${containers} ${childType}${containers !== 1 ? (container.containerType === 'ROOM' ? 'e' : 'n') : ''}`,
      )
    }
    if (items > 0) {
    ...
    if (parts.length === 0) {
      return cfg
        ? `Leere${container.containerType === 'ROOM' ? 'r Raum' : container.containerType === 'SHELF' ? 's Regal' : ' Kiste'}`
        : 'Leer'
    }
    return parts.join(' · ')
  }
  ...
  >
    ...
    <div v-if="roots.length > 0" class="px-4 space-y-2 pb-4">
      <a
        ...
        :style="
          isSelecting && selectedIds.includes(container.id)
      ...
      >
        ...
        >
          <Icon
            v-if="selectedIds.includes(container.id)"
            icon="mdi:check"
        ...
        >
          <Icon
            :icon="
              containerConfig[container.containerType as keyof typeof containerConfig]?.icon ??
              'mdi:cube-outline'
            "
            class="w-6 h-6"
            style="color: var(--color-accent)"

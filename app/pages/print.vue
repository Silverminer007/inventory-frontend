...
import { getBreadcrumb, containerConfig } from '~/utils/containerUtils'
...
const containerIds = ((route.query.containers as string) ?? '')
...
const SIZE_CONFIG: Record<PrintSize, { perPage: number; cols: number; qrSize: number; namePx: number }> = {
      ...
      <div class="flex gap-2">
        <button class="toolbar-btn" onclick="window.print()">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18 3H6v4H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2v4h12v-4h2a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2V3zm-2 2v2H8V5h8zM8 19v-6h8v6H8zm10-8a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
          </svg>
          Drucken
        </button>
    ...
    <template v-else>
      ...
      >
        ...
        >
          ...
          >
            <!-- Type badge -->
            <div class="lbl-badge">{{ label.typeLabel }}</div>

            <!-- Name (JS-fitted) -->
            <div class="lbl-name-wrap">
              <div class="label-name lbl-name">{{ label.name }}</div>
            </div>

            <!-- Path -->
            <div v-if="label.path" class="lbl-path">{{ label.path }}</div>

            <!-- Position -->
            <div v-if="label.position && size !== 'A7'" class="lbl-position">📍 {{ label.position }}</div>

            <!-- QR code -->
            <div class="lbl-qr">
              <QrCode :value="label.qrUrl" :size="cfg.qrSize" />
              <div v-if="size !== 'A7'" class="lbl-url">{{ label.qrUrl }}</div>
            </div>
          </div>
        </div>
      </div>
...
.print-root { min-height: 100vh; display: flex; flex-direction: column; }
...
.toolbar-info { font-size: 0.875rem; color: #555; }
.toolbar-btn {
...
.toolbar-btn.secondary { background: transparent; color: #555; border: 1px solid #ccc; }
...
.lbl-name-wrap { width: 100%; overflow: hidden; text-align: center; }
.lbl-name {
...
.loading {
  ...
  justify-content: center; font-size: 1rem; color: #666;
}
...
@media print {
  ...
  .size-a4 .label-grid { grid-template-rows: 1fr; }
  /* A5: 2 labels stacked */
  .size-a5 .label-grid { grid-template-rows: 1fr 1fr; }
  /* A6: 2×2 */
  .size-a6 .label-grid { grid-template-rows: 1fr 1fr; }
  /* A7: 2×4 */
  .size-a7 .label-grid { grid-template-rows: 1fr 1fr 1fr 1fr; }
  ...
  .lbl-path { font-size: 9pt; }
  .lbl-position { font-size: 8pt; }
  .lbl-url { font-size: 6pt; color: #999; }
}
</style>

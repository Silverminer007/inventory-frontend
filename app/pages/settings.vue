...
const canConfirmClear = computed(() =>
  stats.value.pending === 0 || confirmedDataLoss.value
)
  ...
  <div class="max-w-xl mx-auto px-4 py-6 space-y-6">
    ...
    <section class="card space-y-3">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="text-sm font-medium" style="color: var(--color-text-primary)">
          {{ sync.pendingCommandCount.value }} Änderung{{ sync.pendingCommandCount.value !== 1 ? 'en' : '' }}
        </span>
      </div>
    ...
    <section class="card space-y-3">
      ...
      <div class="space-y-2 text-sm" style="color: var(--color-text-secondary)">
        <div class="flex justify-between">
          <span>Container</span>
          <span data-testid="container-count" class="font-medium tabular-nums" style="color: var(--color-text-primary)">
            {{ stats.containers }}
          </span>
        </div>
        <div class="flex justify-between">
          <span>Items</span>
          <span data-testid="item-count" class="font-medium tabular-nums" style="color: var(--color-text-primary)">
            {{ stats.items }}
          </span>
        </div>
        <div class="flex justify-between">
          <span>Bilder (Metadaten)</span>
          <span data-testid="image-count" class="font-medium tabular-nums" style="color: var(--color-text-primary)">
            {{ stats.images }}
          </span>
        </div>
        <div class="flex justify-between">
          <span>Ausstehende Commands</span>
          <span data-testid="pending-count" class="font-medium tabular-nums" style="color: var(--color-text-primary)">
            {{ stats.pending }}
          </span>
        </div>
      </div>

      <ErrorBanner v-if="cache.clearError.value" :message="cache.clearError.value" @dismiss="cache.clearError.value = null" />
    ...
    <section class="card space-y-2">
      ...
      <div class="flex justify-between text-sm">
        ...
        <span class="font-mono text-xs truncate max-w-[180px]" style="color: var(--color-text-primary)">
  ...
  <BottomSheet v-if="showClearConfirm" title="Cache leeren?" @close="showClearConfirm = false">
    <div class="space-y-4">
      <p class="text-sm" style="color: var(--color-text-secondary)">
        Alle lokalen Daten werden gelöscht und neu vom Server geladen.
        Online-Verbindung ist erforderlich.
      </p>
      ...
      >
        <p class="text-sm font-semibold" style="color: var(--color-danger-text)">
          Du hast {{ stats.pending }} ungesendete
          Änderung{{ stats.pending !== 1 ? 'en' : '' }}.
      ...
      <button class="btn btn-secondary w-full" @click="showClearConfirm = false">
        Abbrechen
      </button>
    </div>
  </BottomSheet>
